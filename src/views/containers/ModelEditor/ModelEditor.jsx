...
class _ModelEditor extends React.Component {
    ...
    if (
      ...
      if (this.toolManager.stats.err) {
        ...
        if (Array.isArray(this.toolManager.stats.errValues)) {
          missingMats = this.toolManager.stats.errValues
            .map((v) => _t(v))
            .join(', ');
        }
        ...
        this.setState({
          ...
          reviewError: {
            code: 'general_error',
            data: _t(this.toolManager.stats.err, { materials: missingMats }),
          },
      ...
      this.props.dispatch(
        ModelActions.VALIDATE_MODEL.init.func({
  ...
  renderError() {
    const { code, data, nextFunc } = this.state.reviewError;
    const { _t } = this.props;

    let content = null;

    if (code === 'general_error') {
      content = <div className="header">{_t(data)}</div>;
    } else if (code === window.RESULT_CODE.ERROR_CUBEGO_SHAPE_MATCH) {
      content = (
        <React.Fragment>
          <div className="header">{_t('err.shape_already_registered')}</div>
          <div className="matched-gons">
            {data.match_cubegons.map((gon, idx) => (
              <div className="matched-gon" key={idx}>
                <img
                  src={LogicUtils.GetImageFromGonID(gon.id)}
                  onClick={() => {
                    Utils.OpenInNewTab(`/${URLS.CUBEGONS}/${gon.id}`);
                  }}
                />
                <p>
                  {_t('ID')}
                  ...
                  :
                  {' '}
                  {LogicUtils.ConvertShapeDiffToSimilarity(gon.shape_diff)}
                  %
                </p>
              </div>
            ))}
            <div className="notice">{_t('shape_matched_notice')}</div>
          </div>
        </React.Fragment>
      );
      ...
      content = (
        <React.Fragment>
          <div className="header">{_t('err.model_already_registered')}</div>
          <div className="matched-gons">
            {(data.match_cubegons || []).map((gon, idx) => (
              <div className="matched-gon" key={idx}>
                <img
                ...
                <p>
                  ...
                  :
                  {' '}
                  {LogicUtils.ConvertColorDiffToSimilarity(gon.color_diff)}
                  %
                </p>
              </div>
            ))}
            <div className="notice">{_t('model_matched_notice')}</div>
          </div>
        </React.Fragment>
      );
    } else if (code === window.RESULT_CODE.SUCCESS) {
      const gon = data.closest_shape;
      content = (
        <React.Fragment>
          <div className="header">{_t('closest shape')}</div>
          <div className="matched-gons">
      ...
      content = <div className="header">{_t('err.model_not_valid')}</div>;
    }

    return <div className="model-editor__error">{content}</div>;
  }
  ...
  render() {
    ...
    let colorNote = null;
    const numSelectedCubes = 0;
    const numSelectedCubesUsed = (this.toolManager.stats.materials || {})[selectedMaterial.material_id]
      || 0;
    if (
      numSelectedCubes - numSelectedCubesUsed < 0
      && selectedMaterial.is_for_sale
    ) {
      colorNote = (
      ...
      colorNote = (
        <div className="model-editor__color-note error">
          {selectedMaterial.material_id === 0
            ? _t('color.overused_error_plastic', {
              type: _t(selectedMaterial.name),
            })
    ...
    const cubegonTierLength = LogicUtils.CalculateLengthBaseOnTier(
      this.toolManager.stats.gonTier.showPoints,
    );
            ...
            <div className="model-editor__stats">
              <div className="material">
                <div className="total">
                  ...
                  {' '}
                  <span>
                    {this.toolManager.stats.total}
                    <span>
                      /
                      {Config.CUBEGON_MAX_CUBE}
                    </span>
                  </span>
                </div>

                {ObjUtils.Map(
                  this.toolManager.stats.materials,
                  (materialId, count) => {
                    const cube = CUBE_MATERIALS[materialId];
                    return (
                      <div
                        key={materialId}
                        className="cube"
              ...
              <div className="stats">
                ...
                >
                  <img
                    src={require('../../../shared/img/icons/icon-stats.png')}
                  />
                  {this.toolManager.stats.power
                    ? this.toolManager.stats.power[0]
                      !== this.toolManager.stats.power[1]
                      ? `${this.toolManager.stats.power[0]} - ${this.toolManager.stats.power[1]}`
                      : `${this.toolManager.stats.power[0]}`
                    : ''}
                  {this.toolManager.stats.stats_distribute
                  && this.state.showStatsDistribute ? (
                    <div className="stats-distribute">
                      <div className="content">
                        <div className="left">
                          <TextImage
                            order={TextImage.order.REVERSE}
                            text={_t('health')}
                            imgSource={require('../../../shared/img/inventory/health-icon.png')}
                          />
                        </div>
                        <div className="right">
                      ...
                      <div className="content">
                        <div className="left">
                          <TextImage
                            order={TextImage.order.REVERSE}
                            text={_t('attack')}
                            imgSource={require('../../../shared/img/inventory/attack-icon.png')}
                          />
                        </div>
                        <div className="right">
                      ...
                      <div className="content">
                        <div className="left">
                          <TextImage
                            order={TextImage.order.REVERSE}
                            text={_t('defense')}
                            imgSource={require('../../../shared/img/inventory/defense-icon.png')}
                          />
                        </div>
                        <div className="right">
                      ...
                      <div className="content">
                        <div className="left">
                          <TextImage
                            order={TextImage.order.REVERSE}
                            text={_t('speed')}
                            imgSource={require('../../../shared/img/inventory/speed-icon.png')}
                          />
                        </div>
                        <div className="right">
                ...
                <div
                  className="stat"
                  tooltip={
                    Number.isNaN(parseInt(totalCost))
                      ? undefined
                      : _t('estimated cost')
                  }
                  tooltip-position="bottom"
                >
                  <img
                    src={require('../../../shared/img/icons/icon-ether.png')}
                  />
                  {totalCost}
                </div>
              </div>
            ...
            <div className="model-editor__tier-bar">
              <div
                ...
                style={{
                  width: `${
                    cubegonTierLength
                      ? cubegonTierLength.idx * 25
                        + cubegonTierLength.length * 100 * 0.25
                      : 0
                  }%`,
                }}
              />
              {[
                ...
                <div
                  key={idx}
                  className={`tier ${
                    this.toolManager.stats.gonTier.id === tier.id
                      ? 'active'
                      : ''
                  } ${tier.name}`}
                  style={{ left: `${25 * idx}%` }}
                ...
                <div className="model-editor__3d">
                  ...
                  {this.toolManager.stats.err ? (
                    <div className="model-editor__model-error">
                      <Image img="icon_warning" />
                      {' '}
                      <p>
                        {_t(this.toolManager.stats.err, {
                          materials: missingMats,
                        })}
                      </p>
                    </div>
                  ) : null}
