...
export class BabylonArcRotateCamera extends BabylonComponent {
  static create({ scene, canvas }, props) {
    ...
    const babylonCamera = new BABYLON.ArcRotateCamera(props.name || 'camera', props.alpha || 0, props.beta || 0, props.radius || 10,
    ...
    if (props.attachControl) babylonCamera.attachControl(canvas, false, true);
    return camera;
  }
  ...
  beginAnimation(name, keys, prop) {
    const anim = new BABYLON.Animation(name, prop, 60, BABYLON.Animation.ANIMATIONTYPE_FLOAT, BABYLON.Animation.ANIMATIONLOOPMODE_CYCLE);
    anim.setKeys(keys);
