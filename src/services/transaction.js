...
export const PurchasePackage = (dispatch, action, _t, {
  name, address, numPacks, packId, amount, purchaseType, currency, history,
}, finishCallback) => {
  dispatch(action({
    ...
    submitFunc: (obj, callback) => {
      ...
      // Sending Txn
      const cbFunc = (code, data) => defaultCallbackFunction(code, data, callback);
...
export const SubmitModel = (dispatch, action, _t, {
  cubegon_name, cubegon_structure, cubegon_image, num_cubes, energy_limit, total_cost, address, successCallback, failedCallback, finishCallback,
}) => {
  let forceToSubmittingState; let
    fields_order;
  const currentTimestamp = Utils.GetCurrentUnixTime();
  const messageToSign = `cubego-create-${currentTimestamp}`;
...
export const UpdateCubegonName = (dispatch, action, _t, {
  cubegon_name, address, id, tokenId, successCallback, failedCallback, finishCallback,
}) => {
  let forceToSubmittingState; let
    fields_order;
  const messageToSign = `${id}-${''}`;
...
export const RegisterModelToBlockchain = (dispatch, action, _t, {
  retrying, cubegon_name, num_cubes, txn_data, history, successCallback, failedCallback, finishCallback,
}) => {
  dispatch(action({
...
export const DeleteModel = (dispatch, action, _t, {
  tokenId, name, successCallback, failedCallback, finishCallback,
}) => {
  dispatch(action({
...
export const UpdateCubegonEnergy = (dispatch, action, _t, {
  name, tokenId, energyLimit, successCallback, failedCallback, finishCallback,
}) => {
  dispatch(action({
    ...
    fields: {
      ...
      energyList: {
        ...
        options: Object.keys(ENERGY_LIMIT_PRICE).filter((k) => k > energyLimit).map((item) => ({ content: `${item} (${ENERGY_LIMIT_PRICE[item]} ETH)`, value: item })),
        type: 'dropdown',
...
export const TransferCubegon = (dispatch, action, _t, {
  name, fromAdd, token_id, successCallback, failedCallback, finishCallback,
}) => {
  dispatch(action({
...
export const ClaimAirDrop = (dispatch, action, _t, {
  userId, name, successCallback, failedCallback, finishCallback,
}) => {
  dispatch(action({
    title: _t('claim_gift'),
...
export const TransferMaterialCube = (dispatch, action, _t, {
  cubeName, fromAdd, numCubes, successCallback, failedCallback, finishCallback,
}) => {
  dispatch(action({
